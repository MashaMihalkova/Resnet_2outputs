...
def test_model(_model, _test_dataloader, _device):
    ...
    mean_prediction = max(set(mean_prediction), key=mean_prediction.count)

    predicted_classes = tensors2array_classes(arr_tensors_classes)  # массив предсказанных классов

    return predicted_classes, mean_prediction
...
def plot_confusion_matrix_norm(df_confusion, title='Confusion matrix', cmap=plt.cm.Blues, path_to_save_img: str = '') -> None:
    plt.matshow(df_confusion, cmap=cmap)  # imshow
    # plt.title(title)
    plt.colorbar()
    tick_marks = np.arange(len(df_confusion.columns))
...
try:
    ...
    info(f'dir = {dir} \n Loaded {len(test_dataset)} images.')
    # for i in classes_ids_dict.values():
    # k = test_dataloader[i]
    predicted_classes, mean_pred_classes = test_model(model, test_dataloader, device)

    print_results_as_table(OUTPUT_RESULTS)

    targets = []
    for i in range(len(test_dataset.imgs)):
        targets.append(test_dataset.imgs[i][1])
    ...
    print('mean_pred_classes = ', mean_pred_classes)
    print('target_classes = ', targets[0])
    print('acc mean_pred_classes = ',np.sum(np.asarray(targets[0]) == np.asarray(mean_pred_classes)))

    targets_new = []
    predicted_new = []
    for i in range(len(targets)):
        targets_new.append(classes_ids_dict_meta[targets[i]])
        predicted_new.append(classes_ids_dict_meta[predicted_classes[i]])

    print(np.sum(np.asarray(targets_new) == np.asarray(predicted_new)) / len(predicted_new))
