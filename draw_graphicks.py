...
def plot_train_history(train, val, type: str, test):
    ...
    title = type
    title = "Точность сети ResNet-18"
    # plt.plot(epochs, train_acc_np, 'b', label=f'Training {type}')
    # plt.plot(epochs, val_acc_np, 'r', label=f'Validation {type}')
    plt.plot(epochs, train_acc_np, 'b', label=f'Тренировочная выборка')
    plt.plot(epochs, val_acc_np, 'r', label=f'Валидационная выборка')
    if test is not None:
        # plt.plot(epochs, test_acc_np, 'g', label=f'Test {type}')
        plt.plot(epochs, test_acc_np, 'g', label=f'Тестовая выборка')
    plt.title(title)
    plt.xlabel('Эпоха')
    plt.ylabel('Точность')
...
# Path = "D:\\Projects\\bacteria_recognitions\\saved_weights\\NEW_DATA\\RESNET_2OUTPUTS\\seed7_ResNet_2out_input_b23_wd5_q0_u0-size_1024.txt"
# Path = "D:\\Projects\\bacteria_recognitions\\saved_weights\\graphiks\\repeats_maxpooling_resnet_april\\RESNET\\seed6_ResNet_WITH_DP_LIN_weight_decay5_val_without_colorjitter_input-size_1024.txt"
Path = "D:\\Projects\\bacteria_recognitions\\saved_weights\\graphiks\\repeats_maxpooling_resnet_april\\RESNET\\REPLAY_seed5_ResNet_ORIG_input-size_1024.txt"
# Path = "D:\\Projects\\bacteria_recognitions\\saved_weights\\graphiks\\repeats_maxpooling_resnet_april\\RESNET\\TOTAL\\Resnet_RM_dropout\\5_seed99_Resnet_input_b23_wd5_q0_u1-size_1024.txt"
if "exp_Lera" in Path:
    Path_test = Path[:-4]+"_test.txt"
    flag_resnet_2output = 0
elif "Original_ResNet" or "ResNet_input" in Path :
    Path_test = ''
    flag_resnet_2output = 0
else:
    Path_test = ''
